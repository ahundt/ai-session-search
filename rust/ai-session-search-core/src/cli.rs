...
/// analytics, and expert tools without reading twenty-six one-line summaries.
const ROOT_COMMAND_GROUPS: &str = "\
Start here (everyday):   search, messages, show, list, resume, files
Export and maintenance:  export, reindex, doctor, integrations, permissions, package, config, dates
...
struct Cli {
    ...
    /// Protected integration launch binding; generated by `integrations install`.
    #[arg(long = "launch-permission-profile", global = true, hide = true)]
    launch_permission_profile: Option<String>,
    /// Protected harness binding generated by a managed integration adapter.
    #[arg(long = "launch-caller-harness", global = true, hide = true, value_enum)]
    launch_caller_harness: Option<crate::search_scope::CallerHarness>,
    /// Unverified diagnostic declaration; it can narrow but never authorize access.
    #[arg(long, global = true, value_enum, hide = true)]
    declare_caller_harness: Option<crate::search_scope::CallerHarness>,
    /// Unverified diagnostic declaration; it can narrow but never authorize access.
    #[arg(long, global = true, hide = true)]
    declare_caller_model_id: Option<String>,
    /// Unverified diagnostic declaration; it can narrow but never authorize access.
    #[arg(long, global = true, hide = true)]
    declare_caller_working_directory: Option<PathBuf>,
    /// Explicit workspace selector for a bounded permission request or approved retry.
    #[arg(long, global = true, hide = true)]
    permission_workspace_root: Option<PathBuf>,
    /// Approved temporary grant for `--permission-workspace-root`.
    #[arg(
        long,
        global = true,
        hide = true,
        requires = "permission_workspace_root"
    )]
    permission_grant: Option<String>,
    /// Caller-stable operation identity used for atomic grant admission.
    #[arg(long, global = true, hide = true, requires = "permission_grant")]
    permission_operation_id: Option<String>,
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    /// Configure, validate, and explain search-result permissions.
    ...
    #[command(display_order = 26, subcommand)]
    Config(ConfigCmd),
...
impl RootOption {
    fn flag(self) -> &'static str {
        match self {
            ...
            Self::LaunchPermissionProfile => "--launch-permission-profile",
        }
    }
    ...
    fn is_present(self, cli: &Cli) -> bool {
        match self {
            ...
            Self::LaunchPermissionProfile => cli.launch_permission_profile.is_some(),
        }
    }
}
...
#[derive(Debug, Subcommand)]
enum PermissionsCmd {
    /// Show the selected boundary; use `aise permissions validate` to check the full policy.
    Status(PermissionsStatusArgs),
    /// Validate every rule; use `aise permissions status` to inspect active durable state.
    Validate(PermissionsFormatArgs),
    /// Draft a safe profile; then run `aise permissions validate` before using it.
    Init(PermissionsInitArgs),
    /// Explain a decision; use `aise permissions status` for the selected live profile.
    Explain(PermissionsExplainArgs),
    /// Create, remove, or select profiles; verify with `aise permissions validate`.
    Profile {
        #[command(subcommand)]
        command: PermissionProfileCmd,
    },
    /// Add, replace, or remove rules; inspect outcomes with `aise permissions explain`.
    Rule {
        #[command(subcommand)]
        command: PermissionRuleCmd,
    },
    /// Approve one pending request; inspect it afterwards with `aise permissions status`.
    Grant(PermissionGrantArgs),
    /// Revoke one grant; verify removal with `aise permissions status`.
    Revoke { grant_id: String },
    /// Intersect a restrictive profile; inspect overlays with `aise permissions status`.
    Activate(PermissionActivateArgs),
    /// Remove one overlay; verify removal with `aise permissions status`.
    Deactivate { overlay_id: String },
}

#[derive(Debug, Args)]
struct PermissionsStatusArgs {
    /// Include resolved profile and boundary details.
    #[arg(long)]
    effective: bool,
    /// Output format.
    #[arg(long, value_enum, default_value_t = PermissionOutputFormat::Plain)]
    format: PermissionOutputFormat,
}

#[derive(Debug, Args)]
struct PermissionsFormatArgs {
    /// Output format.
    #[arg(long, value_enum, default_value_t = PermissionOutputFormat::Plain)]
    format: PermissionOutputFormat,
}
...
enum PermissionPreset {
    ...
    Directory,
    #[value(name = "current-directory-locked")]
    DirectoryLocked,
    #[value(name = "current-repository")]
    Repository,
}

#[derive(Debug, Args)]
struct PermissionsInitArgs {
    /// Required preset; no preset is inferred when omitted.
    #[arg(long, value_enum)]
    preset: PermissionPreset,
    /// Required profile name; no name is inferred when omitted.
    #[arg(long)]
    profile: String,
    /// Print the exact proposed TOML without writing it.
    #[arg(long)]
    dry_run: bool,
}

#[derive(Debug, Args)]
struct PermissionsExplainArgs {
    /// Profile to simulate. Omit to use the configured default profile.
    #[arg(long)]
    profile: Option<String>,
    /// Required operation to simulate; no operation is inferred when omitted.
    #[arg(long, value_enum)]
    operation: crate::search_scope::SearchOperation,
    /// Required workspace root to simulate; no path is inferred when omitted.
    #[arg(long)]
    session_workspace_root: PathBuf,
...
struct PermissionGrantArgs {
    request_id: String,
    /// Admitted operation count. Omit for one admitted operation.
    #[arg(long, value_parser = parse_permission_uses)]
    uses: Option<NonZeroU64>,
    /// Grant lifetime such as 10m. Omit for a use-bounded grant.
    #[arg(long, conflicts_with_all = ["until", "permanent"])]
    expires_in: Option<String>,
    /// Trusted lifecycle boundary. Omit when no adapter supplies that lifecycle.
    #[arg(long, value_enum, conflicts_with_all = ["uses", "expires_in", "permanent"])]
    until: Option<crate::search_scope::GrantUntil>,
    #[arg(long, conflicts_with_all = ["uses", "expires_in", "until"], requires = "profile")]
    permanent: bool,
    /// Profile used only for permanent conversion. Omit for a temporary grant.
    #[arg(long)]
    profile: Option<String>,
}

#[derive(Debug, Args)]
struct PermissionActivateArgs {
    profile: String,
    /// Overlay lifetime. Omit only when --until supplies a trusted lifecycle.
    #[arg(long, required_unless_present = "until", conflicts_with = "until")]
    expires_in: Option<String>,
    /// Trusted lifecycle boundary. Omit to use --expires-in.
    #[arg(long, value_enum, conflicts_with = "expires_in")]
    until: Option<crate::search_scope::GrantUntil>,
}

#[derive(Debug, Default, Args)]
struct PermissionMutationControl {
    /// Print the exact resulting permission panel without writing it.
    #[arg(long)]
    dry_run: bool,
}

#[derive(Debug, Subcommand)]
enum PermissionProfileCmd {
    Create(PermissionProfileCreateArgs),
    Remove {
        profile: String,
        #[command(flatten)]
        control: PermissionMutationControl,
    },
    SetDefault {
        profile: String,
        #[command(flatten)]
    ...
    SetDefaultEffect {
        profile: String,
        #[arg(value_enum)]
        effect: crate::search_scope::ProfileDefault,
        #[command(flatten)]
        control: PermissionMutationControl,
    },
    List,
...
#[derive(Debug, Args)]
struct PermissionProfileCreateArgs {
    profile: String,
    #[arg(long, value_enum)]
    default: crate::search_scope::ProfileDefault,
    #[command(flatten)]
    control: PermissionMutationControl,
}

#[derive(Debug, Subcommand)]
enum PermissionRuleCmd {
    Add(PermissionRuleMutationArgs),
    Replace(PermissionRuleReplaceArgs),
    Remove {
        profile: String,
        rule_id: String,
        #[command(flatten)]
    ...
    Exception {
        #[command(subcommand)]
        command: PermissionExceptionCmd,
    },
}

#[derive(Debug, Args)]
struct PermissionRuleMutationArgs {
    profile: String,
    #[arg(long)]
    rule_id: String,
    #[arg(long, value_enum)]
    effect: crate::search_scope::RuleEffect,
    #[arg(long)]
    session_workspace_root: Vec<PathBuf>,
    #[arg(long, value_enum)]
    operation: Vec<crate::search_scope::SearchOperation>,
    #[command(flatten)]
    control: PermissionMutationControl,
}

#[derive(Debug, Args)]
struct PermissionRuleReplaceArgs {
    profile: String,
    rule_id: String,
    #[arg(long, value_enum)]
    effect: crate::search_scope::RuleEffect,
...
#[derive(Debug, Subcommand)]
enum PermissionExceptionCmd {
    Add(PermissionExceptionMutationArgs),
    Remove {
        profile: String,
        rule_id: String,
        exception_id: String,
...
#[derive(Debug, Args)]
struct PermissionExceptionMutationArgs {
    profile: String,
    rule_id: String,
    #[arg(long)]
    exception_id: String,
...
fn parse_permission_duration_seconds(value: &str) -> Result<i64> {
    if value.is_empty() || value.bytes().any(|byte| byte.is_ascii_whitespace()) {
        bail!("expires_in must be a positive duration such as 15m or 1h30m");
    }
    ...
    while index < bytes.len() {
        ...
        if start == index || index == bytes.len() {
            bail!("expires_in {value:?} must use positive d, h, m, s segments in that order");
        }
        let amount: i64 = value[start..index]
            .parse()
            .map_err(|_| anyhow!("expires_in segment is too large in {value:?}"))?;
        if amount <= 0 {
            bail!("expires_in segments must be greater than zero, got {amount}");
        }
        let (rank, multiplier) = match bytes[index] {
            ...
            b'm' => (2, 60_i64),
            b's' => (1, 1_i64),
            _ => bail!("expires_in {value:?} accepts only d, h, m, s units"),
        };
        if rank >= previous_rank {
            bail!("expires_in {value:?} must use each unit once in d, h, m, s order");
        }
        previous_rank = rank;
        total = total
            .checked_add(
                amount
                    ...
                    .ok_or_else(|| anyhow!("expires_in {value:?} is too large"))?,
            )
            .ok_or_else(|| anyhow!("expires_in {value:?} is too large"))?;
        index += 1;
    }
    if total > 30 * 86_400 {
        bail!("expires_in may be at most 30d, got {value:?}");
    }
    Ok(total)
}

fn parse_permission_uses(value: &str) -> std::result::Result<NonZeroU64, String> {
    let parsed = value.parse::<u64>().ok().and_then(NonZeroU64::new);
    match parsed {
        Some(parsed) if parsed.get() <= 1_000_000 => Ok(parsed),
        _ => Err(format!(
            "uses must be an integer from 1 through 1000000, got {value}; omit --uses for one admitted operation"
        )),
    }
}
...
fn command_search_operation(command: &Commands) -> crate::search_scope::SearchOperation {
    ...
    match command {
        ...
        | Commands::Vocab(_)
        | Commands::Repeats(_)
        | Commands::Skills(_) => SearchOperation::Analyze,
        Commands::Export(_) => SearchOperation::Export,
        Commands::Resume(_) => SearchOperation::Resume,
        Commands::Files(crate::files::FilesCmd::Extract(_)) => SearchOperation::RestoreFiles,
        Commands::Doctor(_) => SearchOperation::AdminMetadata,
        Commands::Reindex(_) | Commands::Compact | Commands::RefreshIndex => {
            SearchOperation::IndexAdmin
        }
        Commands::Permissions(_) => SearchOperation::PolicyAdmin,
        Commands::Db(_) => SearchOperation::Schema,
        _ => SearchOperation::Read,
    }
}

struct CliAdmissionClaim {
...
impl Drop for CliAdmissionClaim {
    fn drop(&mut self) {
        if self.active {
            let _ = self.store.finish_claim(
                &self.grant_id,
                &self.operation_id,
                crate::permission_store::ClaimTerminal::Failed,
            );
        }
    }
}
...
fn execute(cli: Cli) -> Result<()> {
    ...
    let launch_permission_profile = cli.launch_permission_profile;
    let launch_caller_harness = cli.launch_caller_harness;
    let declare_caller_harness = cli.declare_caller_harness;
    let declare_caller_model_id = cli.declare_caller_model_id;
    let declare_caller_working_directory = cli.declare_caller_working_directory;
    let permission_workspace_root = cli.permission_workspace_root;
    let permission_grant = cli.permission_grant;
    let permission_operation_id = cli.permission_operation_id;
    ...
    let command = match cli.command {
        Commands::Integrations(IntegrationsCmd::Install(mut args)) => {
            let config_path = Config::selected_config_path(overrides.config_path.clone());
            if let Some(profile) = args.permission_profile.as_deref() {
                let resolved = Config::resolve(overrides.clone())?;
                let permissions = resolved.config.search.permissions.as_ref().ok_or_else(|| {
                    anyhow!(
                        "--permission-profile {profile:?} requires a [search.permissions] panel in {}",
                        resolved.config_path.display()
                    )
                })?;
                require_permission_profile(permissions, profile)?;
                args.permission_config_path = Some(resolved.config_path);
            }
    ...
    let command = match command {
        Commands::Mcp(crate::integrations::McpCmd::Serve) => {
            let mut resolved = Config::resolve(overrides.clone())?;
            if let Some(profile) = launch_permission_profile {
                let permissions = resolved
                    ...
                    .ok_or_else(|| {
                        anyhow!(
                            "--launch-permission-profile {profile:?} requires [search.permissions] in {}",
                            resolved.config_path.display()
                        )
                    })?;
                require_permission_profile(permissions, &profile)?;
                permissions.default_profile = profile;
            }
            let mut policy_inputs = crate::search_scope::TrustedPolicyInputs::default();
            if let Some(harness) = launch_caller_harness {
                policy_inputs = policy_inputs.with_launch_harness(harness);
            }
            if let Some(harness) = declare_caller_harness {
                policy_inputs = policy_inputs.with_declared_harness(harness);
            }
            if let Some(model_id) = declare_caller_model_id {
                policy_inputs = policy_inputs.with_declared_model_id(model_id);
            }
            if let Some(path) = declare_caller_working_directory {
                policy_inputs = policy_inputs.with_declared_working_directory(path)?;
            }
            if resolved.config.search.permissions.is_some() {
                let state_path = permission_state_path(&resolved);
                let (store, generation) = permission_store_generation(&resolved)?;
                resolved
                    .config
                    .search
                    .permissions
                    .as_mut()
                    .ok_or_else(|| anyhow!("permission policy disappeared during MCP setup"))?
                    .permission_state_database = Some(state_path.to_string_lossy().into_owned());
                policy_inputs.overlay_profiles = store.active_overlay_profiles(
                    generation,
                    None,
                    chrono::Utc::now().timestamp(),
                )?;
                policy_inputs.policy_generation = Some(generation);
            }
            return crate::mcp_server::serve_with_config_and_policy_inputs(
                resolved.config,
                policy_inputs,
            );
        }
    ...
    if let Commands::Permissions(PermissionsCmd::Init(args)) = &command {
        let config_path = Config::selected_config_path(overrides.config_path.clone());
        if !config_path.exists() {
            let cwd = std::env::current_dir().context("cannot capture CLI working directory")?;
            let permissions = permissions_from_preset(
                crate::search_scope::SearchPermissionsConfig::default(),
                args.preset,
                &args.profile,
                &cwd,
            )?;
            if args.dry_run {
                print!("{}", permissions_panel_toml(&permissions)?);
            } else {
                apply_permissions_config_to_path(
                    &config_path,
                    permissions,
    ...
    let command = match command {
        Commands::Permissions(command) => return run_permissions_cmd(&resolved, command),
        command => command,
    };
    ...
    if let Commands::Db(cmd) = command {
        if matches!(&cmd, crate::sql_query::DbCmd::Query(_)) {
            crate::search_scope::ensure_search_raw_sql_allowed(&config.search, "aise db query")?;
        }
    ...
    let authorization_operation = command_search_operation(&command);
    let explicit_maintenance = matches!(&command, Commands::Reindex(_) | Commands::Compact);
    if config.search.permissions.is_none()
        && (permission_workspace_root.is_some()
            || permission_grant.is_some()
            || permission_operation_id.is_some())
    {
        anyhow::bail!(
            "permission request arguments require a configured [search.permissions] policy"
        );
    }
    let mut admission_claim = None;
    let mut app = if explicit_maintenance {
        SessionSearch::open_for_maintenance(config.clone())?
    } else if config.search.permissions.is_some() {
        let cwd = std::env::current_dir().context("cannot capture CLI working directory")?;
        let access_inputs =
            crate::search_scope::TrustedAccessInputs::capture(&config.search.scope, Vec::new())?;
        let (store, generation) = permission_store_generation(&resolved)?;
        let overlay_profiles =
            store.active_overlay_profiles(generation, None, chrono::Utc::now().timestamp())?;
        let mut policy_inputs = crate::search_scope::TrustedPolicyInputs::default()
            .with_observed_working_directory(cwd.clone())?;
        if let Some(harness) = launch_caller_harness {
            policy_inputs = policy_inputs.with_launch_harness(harness);
        }
        if let Some(harness) = declare_caller_harness {
        ...
        policy_inputs.policy_generation = Some(generation);
        policy_inputs.overlay_profiles = overlay_profiles;
        policy_inputs.operation = Some(authorization_operation);
        if let Some(root) = permission_workspace_root {
            ...
            let policy = crate::search_scope::EffectiveSearchPolicy::resolve(
                &config.search,
                policy_inputs.clone(),
            )?;
            match policy.preflight_session_selector(&selector) {
                crate::search_scope::PolicyPreflightDecision::Allow => {
                    if permission_grant.is_some() {
                        anyhow::bail!(
                            "--permission-grant was supplied for a selector already allowed by policy"
                        );
                    }
                }
                crate::search_scope::PolicyPreflightDecision::HardBlock => {
                    anyhow::bail!("permission request is hard-blocked and cannot be overridden")
                }
                crate::search_scope::PolicyPreflightDecision::FilterSilently => {
                    if permission_grant.is_some() {
                        anyhow::bail!(
                            "permission selector is ambiguous or outside the grant envelope"
                        );
                    }
                }
                crate::search_scope::PolicyPreflightDecision::Request {
                    max_uses,
                    max_expires_in_seconds,
                    selector_digest,
                    ..
                } => {
                    let caller_binding = policy_inputs.caller_context_binding()?;
                    if let Some(grant_id) = permission_grant {
                        let operation_id = permission_operation_id.ok_or_else(|| {
                            anyhow::anyhow!(
                                "--permission-operation-id is required with --permission-grant"
                            )
                        })?;
                        if store.grant_selector(&grant_id)? != selector {
                            anyhow::bail!("permission grant selector does not match this request");
                        }
                        match store.admit(&crate::permission_store::AdmissionInput {
                            ...
                            crate::permission_store::AdmissionOutcome::Admitted => {}
                            outcome => anyhow::bail!(
                        ...
                        admission_claim = Some(CliAdmissionClaim {
                            store,
                            grant_id,
                            operation_id,
                            active: true,
                        ...
                        let request = store.create_typed_pending_request(
                            crate::permission_store::PendingRequestInput {
                                policy_generation: generation,
                                caller_context_binding: caller_binding,
                                operation: crate::search_scope::SearchOperation::Read,
                                selector_digest,
                            ...
                            crate::permission_store::PermissionRequestBounds {
                                max_uses: NonZeroU64::new(max_uses)
                                    .expect("policy max_uses is nonzero"),
                                max_expires_in_seconds,
                            },
                            chrono::Utc::now().timestamp(),
                        )?;
                        anyhow::bail!(
                            "permission approval required: request_id={} max_uses={max_uses}; approve with `aise permissions grant {}` and retry with --permission-grant plus a stable --permission-operation-id",
                            request.as_str(),
                            request.as_str()
                        );
                    }
        ...
        } else if permission_grant.is_some() || permission_operation_id.is_some() {
            anyhow::bail!(
                "--permission-grant and --permission-operation-id require --permission-workspace-root"
            );
        }
        SessionSearch::open_with_policy_inputs(config.clone(), access_inputs, policy_inputs)?
    } else {
    ...
    match command {
        ...
        Commands::Resume(args) => {
            let session = db.resolve_session_record_for_operation(
                &args.id,
                crate::search_scope::SearchOperation::Resume,
            )?;
        ...
        Commands::Permissions(_) => {
            unreachable!("permissions commands return before opening the index")
        }
    ...
    if let Some(claim) = admission_claim {
        claim.succeed()?;
    }
...
fn run_permissions_cmd(resolved: &ResolvedConfig, command: PermissionsCmd) -> Result<()> {
    use crate::search_scope::{
        EffectiveSearchPolicy, PolicyDecision, PolicyProfileConfig, PolicyResource,
        PolicyRuleConfig, PolicySelectorConfig, SessionPolicyTarget, TemporaryGrantCoverage,
        TrustedPolicyInputs,
    };

    match command {
        PermissionsCmd::Status(args) => {
            let permissions = resolved.config.search.permissions.as_ref();
            let (ceiling, profile, mode) = permissions.map_or(
                ("unrestricted", "unrestricted", "compatibility-default"),
                |permissions| {
                    (
                        permissions.ceiling_profile.as_str(),
                        permissions.default_profile.as_str(),
                        "application-policy",
                    )
                },
            );
            let state = if permissions.is_some() {
                let (store, generation) = permission_store_generation(resolved)?;
                let now = chrono::Utc::now().timestamp();
                store.prune_terminal_state(now, 30 * 24 * 60 * 60)?;
                Some(store.status(generation, now)?)
            } else {
                None
            };
            if args.format == PermissionOutputFormat::Json {
                println!(
                    "{}",
                    serde_json::to_string_pretty(&serde_json::json!({
                        ...
                        "enforcement_strength": "application-only",
                        "direct_database_bypass": true,
                        "state": state,
                    }))?
                );
            } else {
                println!("Permissions: {mode}");
                println!("Ceiling profile: {ceiling}");
                println!("Default profile: {profile}");
                println!(
                    "Enforcement: application-only (same-user code can open the index or transcripts directly)"
                );
                if let Some(state) = state {
                    println!("Pending requests: {}", state.pending_requests);
                    println!("Active grants: {}", state.active_grants);
                    println!("Active overlays: {}", state.active_overlays);
                    println!("In-flight claims: {}", state.in_flight_claims);
                }
            }
            Ok(())
        }
        PermissionsCmd::Validate(args) => {
                ...
                println!(
                    ...
                    serde_json::to_string_pretty(&serde_json::json!({
                        ...
                        "profiles": resolved
                            ...
                            .permissions
                            .as_ref()
                            .map_or(0, |permissions| permissions.profiles.len()),
                    }))?
                );
            } else {
                println!("Permissions valid: {}", resolved.config_path.display());
            }
            Ok(())
        }
        PermissionsCmd::Init(args) => {
            let cwd = std::env::current_dir().context("cannot capture CLI working directory")?;
            let permissions = permissions_from_preset(
                resolved
                    .config
        ...
        PermissionsCmd::Explain(args) => {
            let cwd = std::env::current_dir().context("cannot capture CLI working directory")?;
            let workspace = resolve_management_path(&cwd, &args.session_workspace_root)?;
            let mut search = resolved.config.search.clone();
            if let Some(profile) = args.profile {
                let permissions = search.permissions.get_or_insert_with(Default::default);
                permissions.default_profile = profile;
            }
            let policy = EffectiveSearchPolicy::resolve(
                &search,
                TrustedPolicyInputs::default().with_observed_working_directory(cwd)?,
            )?;
            let transcript = workspace.join(".aise-simulated-transcript.jsonl");
            let decision = policy.evaluate_session(
            ...
            if args.format == PermissionOutputFormat::Json {
                println!(
                    ...
                    serde_json::to_string_pretty(&serde_json::json!({
                        ...
                        "decision": match decision {
                            ...
                            PolicyDecision::HardBlock => "hard-block",
                        },
                        "operation": format!("{:?}", args.operation).to_ascii_lowercase(),
                        "profile": policy.profile_name(),
                        "ceiling_profile": policy.ceiling_name(),
                        "session_provider": args.session_provider.as_str(),
                ...
                println!("Profile: {}", policy.profile_name());
                println!("Simulation only: no search or grant was executed");
            }
            Ok(())
        }
        PermissionsCmd::Profile { command } => {
            let mut permissions = resolved
                .config
                .search
                .permissions
                .clone()
            ...
            let dry_run = match command {
                PermissionProfileCmd::List => {
                    println!("unrestricted (built-in)");
                    for name in permissions.profiles.keys() {
                        println!("{name}");
                    }
                    return Ok(());
                }
                PermissionProfileCmd::Show { profile } => {
                    if profile == "unrestricted" {
                        println!("default = \"allow\" # built-in");
                        return Ok(());
                    }
                    let profile = permissions
                        .profiles
                        .get(&profile)
                        .ok_or_else(|| anyhow!("permission profile is not defined"))?;
                    print!("{}", toml::to_string_pretty(profile)?);
                    return Ok(());
                }
                PermissionProfileCmd::Create(args) => {
                    if args.profile == "unrestricted"
                        || permissions.profiles.contains_key(&args.profile)
                    {
                        bail!(
                            "permission profile {:?} already exists or is reserved",
                            args.profile
                        );
                    }
                    let dry_run = args.control.dry_run;
                ...
                PermissionProfileCmd::Remove { profile, control } => {
                    if profile == permissions.default_profile
                        || profile == permissions.ceiling_profile
                    {
                        bail!(
                            "permission profile {profile:?} is selected; choose another default/ceiling before removing it"
                        );
                    }
                    if permissions.profiles.remove(&profile).is_none() {
                        bail!("permission profile {profile:?} is not defined");
                    }
                    control.dry_run
                }
                PermissionProfileCmd::SetDefault { profile, control } => {
                    require_permission_profile(&permissions, &profile)?;
                    permissions.default_profile = profile;
                    control.dry_run
                }
                PermissionProfileCmd::SetCeiling { profile, control } => {
                    require_permission_profile(&permissions, &profile)?;
                    permissions.ceiling_profile = profile;
                    control.dry_run
                }
                PermissionProfileCmd::SetDefaultEffect {
                    profile,
                    effect,
                    control,
                } => {
                    let profile = permissions
                        ...
                        .get_mut(&profile)
                        .ok_or_else(|| anyhow!("permission profile is not defined"))?;
                    profile.default = effect;
                    control.dry_run
                }
            };
            preview_or_apply_permissions_config(
                &resolved.config_path,
                permissions,
                "modify permission profile",
                dry_run,
            )
        }
        PermissionsCmd::Rule { command } => {
            let cwd = std::env::current_dir().context("cannot capture CLI working directory")?;
            let mut permissions = resolved
                .config
                .search
                ...
                .unwrap_or_default();
            let dry_run = match command {
                PermissionRuleCmd::Add(args) => {
                    let profile = permissions.profiles.get_mut(&args.profile).ok_or_else(|| {
                        anyhow!("permission profile {:?} is not defined", args.profile)
                    })?;
                    if profile
                        .rules
                        .iter()
                        .any(|rule| rule.rule_id == args.rule_id)
                    {
                        bail!(
                            "rule_id {:?} already exists in profile {:?}",
                            args.rule_id,
                            args.profile
                        );
                    }
                    let dry_run = args.control.dry_run;
                    profile.rules.push(policy_rule_from_args(args, &cwd)?);
                    dry_run
                }
                PermissionRuleCmd::Replace(args) => {
                    let profile = permissions.profiles.get_mut(&args.profile).ok_or_else(|| {
                        anyhow!("permission profile {:?} is not defined", args.profile)
                    })?;
                    let rule = profile
                        .rules
                        .iter_mut()
                        .find(|rule| rule.rule_id == args.rule_id)
                        .ok_or_else(|| anyhow!("rule_id {:?} is not defined", args.rule_id))?;
                    let dry_run = args.control.dry_run;
                    *rule = PolicyRuleConfig {
                        rule_id: args.rule_id,
                        effect: args.effect,
                        resource: PolicyResource::Session,
                        exceptions: Vec::new(),
                        selector: PolicySelectorConfig {
                            operation: args.operation,
                            session_workspace_root: resolve_management_paths(
                                &cwd,
                                &args.session_workspace_root,
                            )?,
                ...
                PermissionRuleCmd::Remove {
                    ...
                    profile.rules.retain(|rule| rule.rule_id != rule_id);
                    if profile.rules.len() == before {
                        bail!("rule_id {rule_id:?} is not defined");
                    }
                    control.dry_run
                }
                PermissionRuleCmd::Exception { command } => match command {
                    PermissionExceptionCmd::Add(args) => {
                        let rule =
                            permission_rule_mut(&mut permissions, &args.profile, &args.rule_id)?;
                        if rule
                            .exceptions
                            .iter()
                            .any(|exception| exception.exception_id == args.exception_id)
                        {
                            bail!("exception_id {:?} already exists", args.exception_id);
                        }
                        let dry_run = args.control.dry_run;
                        rule.exceptions
                            .push(crate::search_scope::PolicyExceptionConfig {
                    ...
                    PermissionExceptionCmd::Remove {
                        profile,
                        rule_id,
                        exception_id,
                        control,
                    } => {
                        let rule = permission_rule_mut(&mut permissions, &profile, &rule_id)?;
                        let before = rule.exceptions.len();
                        rule.exceptions
                            .retain(|exception| exception.exception_id != exception_id);
                        if rule.exceptions.len() == before {
                            bail!("exception_id {exception_id:?} is not defined");
                        }
                        control.dry_run
                    }
                },
        ...
        PermissionsCmd::Grant(args) => {
            use std::io::IsTerminal as _;

            if args.permanent {
                bail!(
                    "permanent request conversion requires a typed rule-diff builder; use `aise permissions rule add|replace` so the exact TOML change is reviewable"
                );
            }
            if let Some(until) = args.until {
                bail!(
                    "--until {until:?} requires a trusted connection/session adapter; use --uses or --expires-in from an external terminal"
                );
            }
            if !io::stdin().is_terminal() || !io::stdout().is_terminal() {
                bail!(
                    "permission grants require interactive terminal stdin and stdout; rerun in a human-controlled terminal"
                );
            }
            let now = chrono::Utc::now().timestamp();
            let expires_at_epoch_seconds = args
                .expires_in
                .as_deref()
                .map(parse_permission_duration_seconds)
                .transpose()?
                .map(|seconds| now.saturating_add(seconds));
            println!("Pending request: {}", args.request_id);
            println!("Uses: {}", args.uses.map_or(1, NonZeroU64::get));
            if let Some(expires_at) = expires_at_epoch_seconds {
                println!("Expires at UTC epoch second: {expires_at}");
            }
            if !prompt_confirm("Approve this bounded search permission?")? {
                bail!("permission grant cancelled; no authority was added");
            }
            let store =
                crate::permission_store::PermissionStore::open(&permission_state_path(resolved))?;
            let grant = store.approve_request(
                &args.request_id,
                crate::permission_store::GrantApproval {
                    uses: Some(args.uses.unwrap_or(NonZeroU64::MIN)),
                    expires_at_epoch_seconds,
                },
                now,
            )?;
            println!("Permission grant created: {}", grant.as_str());
            Ok(())
        }
        PermissionsCmd::Revoke { grant_id } => {
            let store =
                crate::permission_store::PermissionStore::open(&permission_state_path(resolved))?;
            store.revoke_grant(&grant_id, chrono::Utc::now().timestamp())?;
            println!("Permission grant revoked: {grant_id}");
            Ok(())
        }
        PermissionsCmd::Activate(args) => {
            use std::io::IsTerminal as _;

            require_permission_profile(
                resolved
                    .config
                    .search
                    .permissions
                    .as_ref()
                    .ok_or_else(|| anyhow!("profile overlays require [search.permissions]"))?,
                &args.profile,
            )?;
            if args.until.is_some() {
                bail!("--until session-end requires a trusted harness session adapter");
            }
            let now = chrono::Utc::now().timestamp();
            let duration = parse_permission_duration_seconds(
                args.expires_in
                    .as_deref()
                    .expect("clap requires expires_in when until is absent"),
            )?;
            if !io::stdin().is_terminal() || !io::stdout().is_terminal() {
                bail!(
                    "restrictive overlay activation requires interactive terminal stdin and stdout"
                );
            }
            println!("Restrictive overlay profile: {}", args.profile);
            println!("Expires in: {} seconds", duration);
            if !prompt_confirm("Activate this restrictive profile overlay?")? {
                bail!("overlay activation cancelled; policy state was not modified");
            }
            let (store, generation) = permission_store_generation(resolved)?;
            let overlay = store.activate_overlay(
                crate::permission_store::OverlayActivation {
                    profile: args.profile,
                    policy_generation: generation,
                    expires_at_epoch_seconds: now.saturating_add(duration),
                    session_binding: None,
                },
                now,
            )?;
            println!("Restrictive overlay activated: {}", overlay.as_str());
            Ok(())
        }
        PermissionsCmd::Deactivate { overlay_id } => {
            use std::io::IsTerminal as _;

            if !io::stdin().is_terminal() || !io::stdout().is_terminal() {
                bail!("restrictive overlay deactivation requires interactive terminal stdin and stdout");
            }
            if !prompt_confirm("Deactivate this restrictive profile overlay?")? {
                bail!("overlay deactivation cancelled; policy state was not modified");
            }
            let store =
                crate::permission_store::PermissionStore::open(&permission_state_path(resolved))?;
            store.deactivate_overlay(&overlay_id, chrono::Utc::now().timestamp())?;
            println!("Restrictive overlay deactivated: {overlay_id}");
            Ok(())
        }
    }
}

fn permissions_from_preset(
    mut permissions: crate::search_scope::SearchPermissionsConfig,
    preset: PermissionPreset,
    profile_name: &str,
    cwd: &std::path::Path,
) -> Result<crate::search_scope::SearchPermissionsConfig> {
    use crate::search_scope::{
        GrantEnvelopeConfig, PolicyProfileConfig, PolicyResource, PolicyRuleConfig,
        PolicySelectorConfig, ProfileDefault, RuleEffect, SearchOperation,
        SessionWorkspaceRelation,
    };

    if permissions.profiles.contains_key(profile_name) || profile_name == "unrestricted" {
        bail!("permission profile {profile_name:?} already exists or is reserved");
    }
    let root = match preset {
        PermissionPreset::Directory | PermissionPreset::DirectoryLocked => {
            resolve_management_path(cwd, cwd)?
        }
        PermissionPreset::Repository => {
            let cwd_text = cwd
                .to_str()
                .ok_or_else(|| anyhow!("current directory is not valid UTF-8"))?;
            let repo = crate::util::find_repo_root(cwd_text).ok_or_else(|| {
                anyhow!(
                    "current-repository requires a Git worktree; use --preset current-directory outside Git"
                )
            })?;
            PathBuf::from(repo)
        }
    };
    let locked = preset == PermissionPreset::DirectoryLocked;
    let relation = matches!(
        preset,
        PermissionPreset::Directory | PermissionPreset::DirectoryLocked
    )
    .then_some(SessionWorkspaceRelation::WithinCallerWorkingDirectory)
    .into_iter()
    .collect();
    let selector = PolicySelectorConfig {
        operation: vec![SearchOperation::Read],
        session_workspace_root: vec![root.to_string_lossy().into_owned()],
        session_workspace_relation: relation,
        ..PolicySelectorConfig::default()
    };
    permissions.profiles.insert(
        ...
        PolicyProfileConfig {
            default: if locked {
                ProfileDefault::HardBlock
            } else {
                ProfileDefault::Block
            },
            rules: vec![PolicyRuleConfig {
                rule_id: match preset {
                    PermissionPreset::Repository => "current-repository",
                    _ => "current-directory",
                }
                .to_owned(),
            ...
            grant_envelope: (!locked).then_some(GrantEnvelopeConfig {
                selector: PolicySelectorConfig {
                    operation: vec![SearchOperation::Read],
                    ..PolicySelectorConfig::default()
                },
                max_uses: NonZeroU64::new(1),
                max_expires_in: None,
                allowed_until: Vec::new(),
            }),
        },
    );
    permissions.default_profile = profile_name.to_owned();
    permissions.validate()?;
    Ok(permissions)
}

fn require_permission_profile(
    permissions: &crate::search_scope::SearchPermissionsConfig,
    profile: &str,
) -> Result<()> {
    if profile != "unrestricted" && !permissions.profiles.contains_key(profile) {
        bail!("permission profile {profile:?} is not defined");
    }
    Ok(())
}

fn policy_rule_from_args(
    args: PermissionRuleMutationArgs,
    cwd: &std::path::Path,
) -> Result<crate::search_scope::PolicyRuleConfig> {
    Ok(crate::search_scope::PolicyRuleConfig {
        rule_id: args.rule_id,
        effect: args.effect,
        resource: crate::search_scope::PolicyResource::Session,
        exceptions: Vec::new(),
        selector: crate::search_scope::PolicySelectorConfig {
            operation: args.operation,
            session_workspace_root: resolve_management_paths(cwd, &args.session_workspace_root)?,
            ..crate::search_scope::PolicySelectorConfig::default()
        },
    })
}

fn permission_rule_mut<'a>(
    permissions: &'a mut crate::search_scope::SearchPermissionsConfig,
    profile: &str,
    rule_id: &str,
) -> Result<&'a mut crate::search_scope::PolicyRuleConfig> {
    permissions
        .profiles
        .get_mut(profile)
        .ok_or_else(|| anyhow!("permission profile {profile:?} is not defined"))?
        .rules
        .iter_mut()
        .find(|rule| rule.rule_id == rule_id)
        .ok_or_else(|| anyhow!("rule_id {rule_id:?} is not defined"))
}

fn permission_store_generation(
    resolved: &ResolvedConfig,
) -> Result<(
    crate::permission_store::PermissionStore,
    crate::permission_store::PolicyGeneration,
)> {
    let store = crate::permission_store::PermissionStore::open(&permission_state_path(resolved))?;
    let bytes = fs::read(&resolved.config_path).with_context(|| {
        format!(
            "cannot read permission config source {}",
            resolved.config_path.display()
        )
    })?;
    let generation = store.observe_valid_policy_source(&resolved.config_path, &bytes)?;
    Ok((store, generation))
}

fn permission_state_path(resolved: &ResolvedConfig) -> PathBuf {
    resolved
        .config
        .search
        .permissions
        .as_ref()
        .and_then(|permissions| permissions.permission_state_database.as_deref())
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            resolved
                .config_path
                ...
                .join("permissions.sqlite")
        })
}

fn resolve_management_paths(cwd: &std::path::Path, paths: &[PathBuf]) -> Result<Vec<String>> {
    paths
        .iter()
        .map(|path| {
            resolve_management_path(cwd, path).map(|path| path.to_string_lossy().into_owned())
        })
        .collect()
}

fn resolve_management_path(cwd: &std::path::Path, path: &std::path::Path) -> Result<PathBuf> {
...
fn permissions_panel_toml(
    permissions: &crate::search_scope::SearchPermissionsConfig,
) -> Result<String> {
    #[derive(Serialize)]
    struct PermissionDocument<'a> {
        search: PermissionSearch<'a>,
    }
    #[derive(Serialize)]
    struct PermissionSearch<'a> {
        permissions: &'a crate::search_scope::SearchPermissionsConfig,
    }

    Ok(toml::to_string_pretty(&PermissionDocument {
        search: PermissionSearch { permissions },
    })?)
}

fn preview_or_apply_permissions_config(
    ...
    apply_permissions_config_to_path(config_path, permissions, action)
}

fn apply_permissions_config_to_path(
    config_path: &std::path::Path,
    permissions: crate::search_scope::SearchPermissionsConfig,
    action: &str,
) -> Result<()> {
    ...
    permissions.validate()?;
    let before = crate::text_file_transaction::snapshot_utf8_regular_file(config_path)?;
    let raw = before
        .as_ref()
        .map_or("", crate::text_file_transaction::TextFileImage::text);
    ...
    if document
        ...
        .is_some_and(|search| search.contains_key("scope"))
    {
        bail!(
            "search.scope and search.permissions cannot both configure authority; remove one panel or run `aise permissions init --dry-run`"
        );
    }
    let panel = permissions_panel_toml(&permissions)?.parse::<toml_edit::DocumentMut>()?;
    document["search"]["permissions"] = panel["search"]["permissions"].clone();
    let after = document.to_string();
    let parsed: Config = toml::from_str(&after)?;
    parsed.validate()?;

    println!("Proposed permission change: {action}");
    println!("Destination: {}", config_path.display());
    print!("{}", permissions_panel_toml(&permissions)?);
    if !io::stdin().is_terminal() || !io::stdout().is_terminal() {
        bail!(
            "permission policy writes require interactive terminal stdin and stdout; rerun in a human-controlled terminal"
        );
    }
    if !prompt_confirm("Apply this permission policy change?")? {
        bail!("permission policy change cancelled; configuration was not modified");
    }
    let change =
        crate::text_file_transaction::TextFileChange::write(config_path.to_owned(), before, after);
    let receipt = crate::integrations::default_transaction_receipt(config_path);
    crate::text_file_transaction::execute_text_file_and_symlink_transaction_with(
        &receipt,
        &[change],
        &[],
        |_, change| crate::text_file_transaction::apply_symlink_change_after(change),
    )?;
    println!("Permission policy updated: {}", config_path.display());
    Ok(())
}
...
fn validate_root_options(cli: &Cli) -> Result<()> {
    ...
    const MCP_INPUTS: &[RootOption] = &[
        RootOption::ConfigFile,
        RootOption::Database,
        RootOption::CacheDirectory,
        RootOption::WorkerThreads,
        RootOption::IndexRefresh,
        RootOption::LaunchPermissionProfile,
    ];
    ...
    let (command_name, allowed) = match &cli.command {
        ...
        Commands::Mcp(_) => ("aise mcp serve", MCP_INPUTS),
        ...
        Commands::Permissions(_) => ("aise permissions", CONFIG_FILE_ONLY),
...
mod tests {
    ...
    /// The root help lists twenty-six commands. `ROOT_COMMAND_GROUPS` sorts them into everyday,
    ...
    #[test]
    fn permissions_cli_parses_the_common_preset_and_transactional_management_grammar() {
        for invocation in [
            vec!["aise", "permissions", "status", "--effective"],
            vec!["aise", "permissions", "validate", "--format", "json"],
            vec![
                "aise",
                "permissions",
                "init",
            ...
            Cli::try_parse_from(&invocation)
                .unwrap_or_else(|error| panic!("{invocation:?} must parse:\n{error}"));
        }
        let root_help = Cli::command().render_help().to_string();
        assert!(root_help.contains("permissions"), "{root_help}");
    }

    #[test]
    fn permission_policy_writes_reject_nonterminal_control_without_touching_config() {
        ...
        let permissions = permissions_from_preset(
            crate::search_scope::SearchPermissionsConfig::default(),
            PermissionPreset::Directory,
            "work",
            &cwd,
        )
        .unwrap();
        let error = apply_permissions_config_to_path(&config_path, permissions, "test write")
            .unwrap_err()
            .to_string();
    ...
    /// against the list itself: 71 characters median before this list reached 26 commands, 104 for the longest
