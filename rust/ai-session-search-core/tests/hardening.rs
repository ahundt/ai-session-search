...
use ai_session_search::models::FileQuery;
...
fn unicode_paths_and_content_round_trip() {
    ...
    let content = ai_session_search::files::reconstruct(
...
fn reconstruct_handles_multibyte_edit_boundaries() {
    ...
    let edits: Vec<_> = db
        ...
        .collect();
    assert_eq!(
        ai_session_search::files::reconstruct(&edits, 2).as_deref(),
        Some("αβγ 🌍 end")
    );
}
...
fn reindex_after_edit_keeps_fts_in_sync_and_drops_stale_content() {
    ...
    assert_eq!(
        db.search_messages(
            "unique-token-aaa",
            &ai_session_search::models::MessageFilters::default()
        )
        .unwrap()
        .len(),
        1
