    ...
    F: Fn(Cursor<Vec<u8>>, &std::path::Path) -> Result<ParsedSession>,
{
    ...
    let offset = ai_session_search::tail::complete_prefix_offset(&path).unwrap();
    let initial_parsed = parse(Cursor::new(initial.as_bytes().to_vec()), &path).unwrap();
    std::fs::write(&path, &final_bytes).unwrap();
    let tail = ai_session_search::tail::tail_parse(&path, offset, &parse)
        .unwrap()
...
fn claude_tail_matches_full_with_cross_boundary_tool_result() {
    ...
    let adapter = ai_session_search::providers::claude::ClaudeAdapter::new(vec![]);
    let initial = concat!(
...
fn cursor_tail_matches_full_with_cross_boundary_tool_result() {
    ...
    let adapter = ai_session_search::providers::cursor::CursorAdapter::new(vec![]);
    let initial = concat!(
...
fn antigravity_tail_matches_full() {
    ...
    let adapter = ai_session_search::providers::antigravity::AntigravityAdapter::new(vec![]);
    let initial = concat!(
...
fn pi_tail_matches_full() {
    ...
    let adapter = ai_session_search::providers::pi::PiAdapter::new(vec![]);
    let initial = concat!(
