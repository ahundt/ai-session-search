...
![aise demo](docs/demo.gif)
<!-- Demo GIF is generated from sanitized sample data (generation scripts kept outside the repo). -->

Read the announcement: [AI Session Search: a local-first memory layer for CLI agents](https://brain.co/blog/aise-a-local-first-memory-layer-for-cli-agents).
...
`aise` keeps recall local. Two small binaries (`aise` and `aise-mcp`), one SQLite file, no daemon. The index is a disposable cache; delete it and rebuild it whenever you want.
...
```bash
git clone git@github.com:braincompany/aise.git
cd aise
...
cargo install --path . --bin aise --locked
cargo install --path . --bin aise-mcp --locked
```
...
merges them automatically (FTS5 `optimize`), and `aise compact` reclaims the freed space on demand
(`optimize` + `VACUUM` + WAL checkpoint; it needs roughly the database's size in free disk while it runs).

## Quick start

```bash
aise list --limit 20        # recent sessions (auto-indexes on first run)
aise search "auth bug"      # keyword search
aise search "redis" --provider codex
aise search "datadog" --provider cursor
aise search "temporal" --provider pi
aise show claude:79accec8-5bf5-415b-a4a5-fe370eb2c998    # defaults to last 40 lines
aise show claude:79accec8-5bf5-415b-a4a5-fe370eb2c998 --summary   # compact purpose/tool/ref/file evidence
aise show claude:79accec8-5bf5-415b-a4a5-fe370eb2c998 --max-lines 0  # full transcript
aise resume 79accec8 --dry-run
aise export 79accec8 --format markdown
aise doctor --format json   # typed schema/parser/provider health + repair commands
aise compact                # reclaim disk space (FTS5 optimize + VACUUM + WAL checkpoint)
aise tui                    # interactive browser
```

## Messages, analytics, and file recovery

Beyond session-level search, `aise` indexes every message — user, assistant, tool
...
aise messages search "race condition" --role assistant --since 2026-01
aise messages search /goal --provider codex --role slash       # exact command text
aise messages search '^/[^[:space:]]+(\s|$)' --regex --role slash  # leading command token
aise messages search -e --path                  # leading-dash literal
aise messages search aise --exclude-path ~/.claude
aise messages search 'TODO|FIXME' --regex --type user
aise messages search "magic values" --fuzzy --type user --since 30d
aise messages search "ls -la" --kind tool-call # invocations only, excluding results
aise messages search "cargo test" --field tool-argument --argument-path /cmd
aise messages search 'https?://|www\.|[[:alnum:].-]+\.[[:alpha:]]{2,}' --regex --refs
aise messages search "citation" --context 3 --refs
aise messages evidence <session-id> --include time-profile # bounded evidence + timing
aise messages get <session-id> --seq 42 --context 5 --refs
aise messages get <session-id>                 # all messages in one session
aise messages timeline <session-id> --seq-from 40 --seq-to 80 --refs

# Analytics
aise corrections --since 7d                    # where you corrected the agent
aise planning --commands '^/[^[:space:]]+$'  # command-token regex; repeat to OR names
aise repeats --since 30d --context 2 --max-groups 25
aise stats --when 2026-01                      # message counts by role

# File recovery (from recorded Write/Edit/MultiEdit/ApplyPatch tool calls)
aise files search '*.rs'                       # files edited, with counts
aise files history src/db.rs                   # ordered versions of one file
aise files extract src/db.rs --version 3 --output-dir /tmp/recovered

aise db schema                              # inspect queryable SQLite tables
aise db schema --table messages             # inspect one table's columns
aise db query 'select role, count(*) from messages group by role'

aise dates                                     # list every supported date/EDTF form
```

`messages evidence` is the first read for a likely session: it returns bounded user-intent, tool, explicit-ref, and changed-file previews plus exact commands for deeper inspection. Add `--include time-profile` for timestamp coverage, observed span, maximum adjacent-message gap, and typed call/result counts. `--refs` extracts URLs, including scheme-less forms such as `docs.rs/linkify`, from returned messages and context windows. Use `--session-id` plus `--seq-from/--seq-to` when you already know the session-local message range. `--role` is canonical; `--type` remains a compatibility alias. `--kind tool-call|tool-result` removes the ambiguity of role `tool`. General argument search uses `--field tool-argument --argument-path <RFC-6901-pointer>` relative to canonical `args`, so uncommon and nested tool schemas require no hard-coded key list. `aise db query` is an expert raw read-only SQL escape hatch over the local AI session-history index; run `aise db schema` first for table and column names.

`aise show` is bounded by default (`[cli].show_max_lines = -40`); pass
`--max-lines 0` only when you intentionally want the entire transcript. For turn-level regex or
fuzzy search, use `aise messages search --regex QUERY` or
`aise messages search --fuzzy QUERY`.
...
`0` means unlimited; output/page defaults are documented in `aise config example`.
...
After installing the binaries, register `aise-mcp` with your MCP clients:
...
| --- | --- | --- | --- |
| Claude Code | `~/.claude.json`, `~/.claude/.mcp.json` | `mcpServers.aise` | `CLAUDE.md` imports `AI_SESSION_SEARCH.md` |
| Claude Desktop | `claude_desktop_config.json` | `mcpServers.aise` | MCP config only |
| Codex CLI / Codex desktop config | `~/.codex/config.toml` | `[mcp_servers.aise]` | managed `AGENTS.md` block |
| Gemini CLI | `~/.gemini/settings.json` | `mcpServers.aise` | MCP config only |
| Antigravity CLI | `~/.gemini/antigravity-cli/settings.json` | `mcpServers.aise` | MCP config only |
| Antigravity legacy | `~/.gemini/antigravity/mcp_config.json` | `mcpServers.aise` | MCP config only |
| Cursor | `~/.cursor/mcp.json` | `mcpServers.aise` | MCP config only |
| Windsurf | `~/.codeium/windsurf/mcp_config.json` | `mcpServers.aise` | MCP config only |
| VS Code | `Code/User/mcp.json` | `servers.aise` with `type = "stdio"` | MCP config only |
| Zed | `Zed/settings.json` | `context_servers.aise` | MCP config only |
| OpenCode | `~/.config/opencode/opencode.json` | `mcp.aise.command[]` | managed `AGENTS.md` block |
| OpenClaw | `~/.openclaw/openclaw.json` | `mcpServers.aise` | MCP config only |
| KiloCode | `Code/User/globalStorage/.../mcp_settings.json` | `mcpServers.aise` | MCP config only |
...
For Claude Code, install also writes `AI_SESSION_SEARCH.md` next to `CLAUDE.md` and adds `@AI_SESSION_SEARCH.md`, using Claude Code's file-import support. For Codex and OpenCode, install adds a short managed block directly to `AGENTS.md` because those harnesses read `AGENTS.md` as literal instructions. Pass `--no-instructions` to skip instruction files. Uninstall removes only the managed `aise` reference or block.
...
```bash
aise mcp install --client codex --dry-run
aise mcp install --client claude
aise mcp install --client claude --no-instructions
aise mcp install --json-mcp-config ~/my-agent/mcp.json
aise mcp install --vscode-config ~/Library/Application\ Support/Code/User/mcp.json
aise mcp install --codex-config ~/.codex/config.toml
aise mcp install --agents-md ~/my-agent/AGENTS.md
aise mcp status
aise mcp uninstall --client codex --dry-run
```
...
aise-mcp install
aise-mcp status
aise-mcp uninstall --client codex
```
...
```bash
claude mcp add --scope user --transport stdio aise -- aise-mcp
```

#### Codex CLI

```bash
codex mcp add aise -- aise-mcp
```
...
Optional config file: `config.toml` under the platform config directory. If it is absent, aise uses built-in defaults. Use `aise config path` to print the exact location, `aise config example` to print the commented reference config, `aise config init` to write it, `aise config show` to print the effective merged TOML, and `aise paths` to see active data paths.
...
Filter Claude Code with `--provider claude` and Claude Desktop local agent sessions with `--provider claude-desktop`. Claude Desktop defaults use the platform config/data directories when available; on Windows that is expected to resolve under `%APPDATA%\Claude`, but use `aise paths` or an absolute custom path to confirm your machine.
