...
| `find_corrections`, `analyze_planning_usage`, `get_statistics` | `AnalysisService::{run_skill,planning,role_statistics}` + `IndexService::status` | The legacy Python implementations are deleted. Message classification is exposed through typed `SkillSelector`, `MessageClassificationQuery`, `SkillRunQuery`, and `SessionSearch.run_skill`; `SkillRunQuery.definition` and `--definition-json` share the same validator and deterministic digest as packaged rules. MCP uses `run_skill_capability` with context-sized pagination and bounded presentation. Rules normally live in adjacent `capability.toml` within a selected skill package. The primary package is the token after `aise skills`; repeatable `--skill` adds packages in evaluation order |
