...
| Date parsing/filtering | removed `parse_date_input`, `_passes_date_filter`, and `FilterSpec` builders | `dates` module plus shared CLI/MCP bounds and PyO3 `DateRange` | Rust exclusively owns date parsing. Session list/search/export/analysis intersect the resolved period with `[created_at, updated_at]`; message/file/event analytics retain scalar event-time filtering. Fixed-clock Rust/PyO3 corpora cover absolute, partial, interval, duration, overlap, and malformed inputs |
...
| Message classification | legacy `find_corrections` plus configurable patterns | `AnalysisService::run_skill` with `SkillSelector`, `SkillRunQuery`, and adjacent `aise-capability.toml`; typed PyO3 records use the same request | Rust canonical for deterministic indexed classification and provider/session/path/date scopes. `SKILL.md` remains harness guidance and is never interpreted by the executor; the legacy false-positive differential corpus remains pending |
...
| Composable Python filters | removed `Filter`, `SearchFilter`, `MessageFilter`, and `FilterSpec` | typed Rust query structs plus immutable PyO3 `QueryScope` and `DateRange` for session and file queries, and `MessageScope` inside a flat `MessageSearchRequest` for message queries | Native requests share structural scope while message-specific predicates stay typed without nested one-use selector wrappers. The legacy `SYSTEM` fallback, duplicate substring predicates, and unused hardcoded 500-character “long” classification were not retained |
...
| MCP | absent in legacy aise | Rust MCP server | One Rust transport over shared services is exposed by Cargo and PyO3 as `aise mcp serve`; installer entries include the same arguments. Protocol/schema/runtime fixtures validate eight advertised tools: `search_sessions`, `get_session`, `list_sessions`, `get_resume_command`, `search_messages`, `run_skill_capability`, `get_index_status`, and `query_session_index`. `run_skill_capability` normally executes the selected package's deterministic adjacent `aise-capability.toml`; a closed typed `definition` can replace only those rules for one call while the selected skill retains identity and authorization. The client or AI harness, not Aise, interprets `SKILL.md`. Classification uses complete authoritative text; MCP delivery defaults to bounded field and match views with exact `(session_id, message_seq)` recovery, while explicit `detail=full` returns the whole message. Every argument object is closed (`additionalProperties=false`) and the runtime validates the same schemas before opening or refreshing the index, so unknown keys, wrong JSON types, invalid enums, and out-of-domain counts fail instead of silently selecting defaults. Text-only tools intentionally omit `outputSchema`; structured tools publish object schemas and matching `structuredContent`. The local cutover validates initialize, advertisement, live structured index status, and portable client registrations with no active sessiongrep MCP keys. |
...
| `find_corrections`, `analyze_planning_usage`, `get_statistics` | `AnalysisService::{run_skill,planning,role_statistics}` + `IndexService::status` | The legacy Python implementations are deleted. Message classification is exposed through typed `SkillSelector`, `MessageClassificationQuery`, `SkillRunQuery`, and `SessionSearch.run_skill`; `SkillRunQuery.definition` and `--definition-json` share the same validator and deterministic digest as packaged rules. MCP uses `run_skill_capability` with context-sized pagination and bounded presentation. Rules normally live in adjacent `aise-capability.toml` within a selected skill package. The primary package is the token after `aise skills`; repeatable `--skill` adds packages in evaluation order |
