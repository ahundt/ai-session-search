...
| Message search/context | `search_messages`, `search_messages_with_context`, `get_messages` | `Db.search_messages`, `message_context`, typed `MessageFilters` | Rust canonical and exposed through PyO3 with asymmetric indexed context; audit every legacy message-type semantic before deleting scans |
...
| Clipboard/session analysis/timeline | `SessionAnalysis`, `timeline_session`, clipboard scan helpers | shared bounded `CatalogService::inspect`, indexed time profile, message timeline/context | Rust inspection is canonical and typed through PyO3; audit only distinct timeline predicates, and postpone platform-specific clipboard interpretation unless measured |
...
| Composable Python filters | `Filter`, `SearchFilter`, `MessageFilter`, `FilterSpec` | typed Rust query structs plus immutable PyO3 `QueryScope` and `DateRangeQuery` | Native message, analysis, and file requests share structural scope; port only remaining useful operation-specific predicates and never pass Python predicates into indexed queries |
