...
| Corrections | `find_corrections` plus configurable patterns | Shared `AnalysisService` and typed PyO3 correction records over `Db.find_corrections` | Rust canonical for indexed classification and structural scopes; remaining date-query binding and legacy false-positive differential corpus are pending |
