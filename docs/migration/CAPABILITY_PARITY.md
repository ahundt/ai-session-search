...
| Export | Python markdown/bulk export | Shared Rust `ExportService`, native CLI, typed PyO3 document, and immutable Rust bundle publication plan | Rust canonical for text/Markdown/JSON single-session export with byte-level compatibility tests. Filtered bulk CLI export uses shared session filters, a bounded configurable default with explicit `limit=0`, portable canonical-ID filenames, and one atomic no-replace directory transaction; typed PyO3 bundle publication remains the next adapter gate |
...
| MCP | absent in legacy aise | Rust MCP server | One Rust transport over shared services is exposed by Cargo and PyO3 as `aise mcp serve`; installer entries include the same arguments. Protocol/schema/runtime fixtures validate eight advertised tools, including bounded read-only analysis, while preserving the prior seven contracts. The previous local cutover validated initialize, those seven tools, live structured index status, and ten portable client registrations with no active sessiongrep MCP keys; installed acceptance of the eighth tool remains in the final local deployment gate |
