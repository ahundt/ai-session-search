...
| MCP | absent in legacy aise | Rust MCP server | One Rust transport over shared services is exposed by Cargo and PyO3 as `aise mcp serve`; installer entries include the same arguments. Protocol/schema/runtime fixtures validate eight advertised tools, including bounded read-only analysis, while preserving the prior seven operations and adding only explicit `limit=0` guidance. The local cutover validates initialize, advertisement of all eight tools, an isolated zero-session `analyze_sessions` call, live structured index status, and ten portable client registrations with no active sessiongrep MCP keys |
