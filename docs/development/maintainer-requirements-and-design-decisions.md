...
| Message-search receipt (`receipt_level=summary` or `full`) | Adds `O(R)` for `corpus`: `filtered_corpus_count` runs `count(*)` over `messages` under the structural predicates, and no index covers the default `kind != 'harness_notice'` predicate, so SQLite reads every row page (`content` precedes `kind` in the row). Measured 2026-08-15 on a 23.7 GB, 2,635,206-message index: `messages search "all_results conflicts with limit" --limit 1` real 0.50 s at `none`, 116.9 s (14 s sys) at `summary`. | `O(1)` application memory beyond the search | Known cost, disclosed in the MCP description and the shipped skill; the search itself is unaffected. Candidate fixes are a covering index on `messages(kind)` (a layout change with a one-time build on existing indexes, so it needs its own migration path) or a bounded corpus figure with an explicit `>=` marker (a receipt-schema change). Neither ships in 1.0.0rc2; do not recommend the receipt for a search that is already slow. |
