...
| Session search | `O(B × (Q + 1) + N + K log K)` aggregate work for positive `K` | `O(K + D_K + batch_budget + D_max)`; explicit all-results makes `K` the number of matches | Bounded batches score on the application worker runtime, but SQLite traversal and global top-K remain serial. A result limit bounds retained hits, not corpus scoring. Preserve deterministic order and compare Rust, Python, CLI, and MCP digests. |
